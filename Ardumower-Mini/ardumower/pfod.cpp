...
void RemoteControl::sendGPSMenu(boolean update){
  ...
  sendSlider("q13", F("Area Soll"), robot->Area_Soll, "", 1, 5, 0); //--------Mähzonen TB-----------   
...
void RemoteControl::sendTimerDetailMenu(int timerIdx, boolean update){
  ...
  sendSlider("p8"+sidx, F("Area "), robot->timer[timerIdx].Area_Timer, "", 1, 5, 0);  //------------Mähzonen TB------------           
...
void RemoteControl::processTimerDetailMenu(String pfodCmd){      
  ...
  if (pfodCmd.startsWith("p0")) robot->timer[timerIdx].active = !robot->timer[timerIdx].active;
    ...
    else if (pfodCmd.startsWith("p8")) { processSlider(pfodCmd, robot->timer[timerIdx].Area_Timer, 1);checkStart = false; }       // -------------Mähzonen TB----------------- 
