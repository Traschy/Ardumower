...
Mower::Mower(){
  ...
  #if defined (ROBOT_ARDUMOWER)
		motorPowerMax              = 20;        // motor wheel max power (Watt)ge�ndert Thorsten 75		  
  ...
  motorMowSpeedMaxPwm        = 200;        // motor mower max PWM ge�ndert thorsten 255
  ...
  // ------ sonar ------------------------------------
  sonarUse                   = 1;          // ge�ndert thorsten use ultra sonic sensor? (WARNING: robot will slow down, if enabled but not connected!)
  sonarLeftUse               = 1;
  sonarRightUse              = 1;
  sonarCenterUse             = 1;          //ge�ndert thorsten
  sonarTriggerBelow          = 30;       // ultrasonic sensor trigger distance (0=off)
	sonarSlowBelow             = 60;     // ultrasonic sensor slow down distance
  
  // ------ perimeter ---------------------------------
  perimeterUse               = 0;          // ge�ndert thorsten use perimeter?    
  ...
  imuUse                     = 0;          // ge�ndert thorsten use IMU?
  ...
  // ------ model R/C ------------------------------------
  remoteUse                  = 0;          // ge�ndert thorsten use model remote control (R/C)?
  ...
  // ------ battery -------------------------------------
    ...
    batMonitor                 = 0;          // monitor battery and charge voltage?
		...
		startChargingIfBelow       = 28.0;      // ge�ndert thorsten 32 start charging if battery Voltage is below (99999=disabled)
		batFull                    = 29.4;      // battery reference Voltage (fully charged) PLEASE ADJUST IF USING A DIFFERENT BATTERY VOLTAGE! FOR a 12V SYSTEM TO 14.4V		
		batFullCurrent             = 0.2;       // ge�ndert thorsten 0.1 current flowing when battery is fully charged	(amp), (-99999=disabled)	
	#else  // ROBOT_MINI
		batMonitor                 = 0;          // monitor battery and charge voltage?
  ...
  // ----- timer -----------------------------------------
  timerUse                   = 1;          // ge�ndert thorsten use RTC and timer?
  ...
  esp8266ConfigString        = "config:FRITZBox,55970298204784759429,192.168.178.90,192.168.178.1,255.255.255.0"; 
  ...
  P1.X = 50.640330; P1.Y = 6.263043;   // ******* NOTE IMPORTANT: eine Area muss immer die Ladestation sein!!!! ****** 

  //----------------   Area 2  ----------------------------

  P2.X = 50.640330; P2.Y = 6.263043;

  //----------------   Area 3  ----------------------------
  
  P3.X = 50.640330; P3.Y = 6.263043;

  //----------------   Area 4  ----------------------------

  P4.X = 50.640330; P4.Y = 6.263043;

  //----------------   Area 5  ----------------------------

  P5.X = 50.640330; P5.Y = 6.263043;
...
//case SEN_MOTOR_MOW: return ADCMan.read(pinMotorMowSense); break;
case SEN_MOTOR_RIGHT: checkMotorFault(); return
ADCMan.read(pinMotorRightSense); break;
case SEN_MOTOR_LEFT: checkMotorFault(); return ADCMan.read(pinMotorLeftSense);break;
//case SEN_MOTOR_MOW_RPM: break; // not used - rpm is updated via interrupt
#endif
