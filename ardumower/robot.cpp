...
//&& (millis() > nextTimeReadArea)) { //read only each 60 Secondes
...
// NOTE IMPORTANT: eine Area muss immer die Ladestation sein!!!! 
  
  radius = 3.0; // Radius um den GPS Punkt in Meter
...
void Robot::loop()  {
  ...
  switch (stateCurr) {
    ...
    case STATE_STATION_FORW:
      ...
      if (millis() >= stateEndTime) setNextState(STATE_checkArea,0);		// Mähzonen TB		        
      break;      
  } // end switch  
